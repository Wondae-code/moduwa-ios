...
struct AccountInfoSection: View {
    ...
    ///
    /// ⚠️ **이 글자를 바꾸면 웹 두 곳과 앱스토어 심사 노트가 함께 틀려진다.** 여기까지 오는 길
    /// `설정 → 프로필 편집 → 회원 탈퇴` 가 세 군데에 **글로 적혀 있다**:
    /// `moduwa.app/support`(회원 탈퇴 안내), `moduwa.app/privacy`(5항, 이용자 권리·삭제),
    /// 그리고 `TestFlight/AppReviewNotes.txt`.
    ///
    /// 심사자는 적힌 대로 따라가 본다 — 경로가 틀리면 "안내대로 했는데 그런 화면이 없다"가 되어,
    /// **5.1.1(v)를 만족시키려 넣은 문장이 반대로 작동한다.** 실제로 2026-09-07 에 방침이
    /// `설정 → 계정 삭제` 로 적혀 있었고(중간의 프로필 편집이 빠지고 라벨도 달랐다), 지원
    /// 페이지가 그것을 베껴 와 세 군데가 함께 틀려 있었다.
    ///
    /// 라벨이나 경로를 바꾸면 **서버팀에 알린다**(양방향 약속, 2026-09-07).
