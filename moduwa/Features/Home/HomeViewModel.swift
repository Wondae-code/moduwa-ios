...
final class HomeViewModel {
    ...
    /// `(like_count + comment_count) desc, created_at desc` 라서(`REVIEW_ORDERS`, app.ts)
    /// 반응 수만 보고 최신성은 동점일 때의 두 번째 키로만 쓰인다 — 반응이 붙는 데는 시간이
    /// 걸리므로 갓 올라온 글은 늘 아래에 선다.
    ///
    /// 실측(2026-09-06 라이브): 추천순 첫 5건이 전부 두 달 전(6월 말~7월 중) 후기였고,
    /// 그날 올라온 후기는 반응이 0 이라 12건 중 뒤로 밀려 **첫 페이지에 아예 없었다.**
    /// QA #4 "최신 피드 안 가져옴"이 가리킨 것이 이 현상이다.
    ///
    /// 기본값을 최신순으로 바꿔 봤다가 **추천 우선으로 되돌렸다.** 그래서 최신 글이 위로
    /// 오지 않는 것은 지금도 그대로다 — 다만 다시 받는 길 둘은 남겨 뒀다:
    /// `refresh()`(당겨서 새로 고침)와 후기 작성 신호(`PostInteractionSignal.reviewWritten`).
    /// 새 글을 위로 올려야 한다면 **정렬 기준 자체**를 손대야 한다(반응 수에 최신성 가중치를
    /// 섞는 식) — 그건 서버 `REVIEW_ORDERS` 를 바꾸는 일이다.
