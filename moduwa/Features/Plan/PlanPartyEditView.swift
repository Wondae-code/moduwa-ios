...
///
/// ⚠️ **지금은 어디서도 열리지 않는다**(2026-09-20). ⋮ 의 "팀 수정" 자리를 "플랜 수정"
/// (`PlanInfoEditView`)이 가져갔다. 동반자를 고칠 길이 앱에 없어졌다는 뜻이다 —
/// 새 플랜 1/6 에서 한 번 고르면 끝이다.
///
/// 파일을 남겨 둔 이유는 **되살릴지 지울지가 아직 정해지지 않아서**다. 되살린다면 갈 자리는
/// `PlanInfoEditView` 안(제목·날짜 옆)이 맞다 — 셋 다 "이 여행이 무엇인가" 를 적는 값이다.
///
