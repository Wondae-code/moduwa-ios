...
struct TravelReview: Identifiable, Hashable, Sendable {
    ...
    /// 이 사람이 **좋아요를 누른 시각**. `liked=true` 목록에서만 실려 온다(서버 2026-09-07).
    ///
    /// 왜 필요한가: 저장 탭은 게시글과 후기를 **한 줄로, 누른 순서로** 보여 준다. 서버는 두
    /// 목록을 각각 이 값으로 정렬해 주지만 목록이 둘이라, 앱이 합치려면 같은 값을 손에
    /// 쥐어야 한다. 없으면 각 묶음은 맞고 둘 사이는 알 수 없다.
    ///
    /// ⚠️ **서버의 `order by` 가 읽는 것과 같은 값이다.** 다르면 앱이 다시 세운 결과가 서버
    /// 순서와 어긋나고, 한 화면 안에서 순서가 흔들린다.
    ///
    /// `liked=true` 가 아닌 목록에는 **필드가 아예 없다** — 늘 실으면 `nil` 이 "안 눌렀다" 인지
    /// "이 목록은 좋아요 목록이 아니다" 인지 갈린다.
    var likedAt: Date? = nil
