...
struct CourseRequest: Sendable {
    ...
    var party: [String]
    /// `car`·`walking`·`wheelchair`(서버 2026-09-20). `MobilityMode.rawValue` 그대로다.
    ///
    /// ⚠️ **이름과 값을 앱의 저장 형식과 똑같이 맞춘 것이다.** 플랜의 `party` 안에 이미
    /// `{"mobilities": ["walking"]}` 로 들어가 있어서, 추천에만 `mobility`(단수) 같은 새
    /// 이름을 만들면 **같은 것을 두 이름으로 부르게** 되고 한쪽을 고칠 때 다른 쪽이
    /// 조용히 어긋난다(서버팀 지적, 2026-09-20).
    ///
    /// 무엇이 달라지나:
    /// - **자차·미전송**은 코스가 같다(하위 호환).
    /// - **뚜벅이·휠체어**는 동선이 좁아진다(가평 39km → 14km, 통영 28km → 5km).
    /// - **자차+휠체어**는 거리는 자차, 접근성은 휠체어 기준이다 — 차로 가도 도착해서는
    ///   휠체어로 다니기 때문이다.
    /// - **휠체어**는 무장애 조사를 받지 않은 곳을 더 세게 민다. 동선을 좁히면 닿는 범위의
    ///   조사된 식당이 줄어 미조사가 그 자리를 메우는데, 무장애 앱이 휠체어 이용자를
    ///   조사 안 된 식당으로 보내는 것은 막아야 할 바로 그 일이라서다.
    ///
    /// 서버는 모르는 값과 빈 배열을 무시한다(400 이 아니다).
    var mobilities: [String]
